/*Hacer un programa que guarde datos de clientes de un banco,
los almacene en cola, y por último muestre los clientes en el
orden correcto.*/
...
int main()
{
	...
	do
	{
		cargar_cliente(c); // Cargarmos cliente
		insertarCola(frente, fin, c); //y luego lo agregamos a cola
		
		cout << "\nDesea agregar mas cliente(s/n): ";
		cin >> rpt;
		cout << "\n";
	} while ((rpt == 'S') || (rpt == 's'));
